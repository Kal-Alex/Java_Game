...
from keras.layers import Dense, Bidirectional,ConvLSTM2D,Conv1D, Attention,TimeDistributed, Dropout, Reshape,LeakyReLU
...
model.add(ConvLSTM2D(filters=16,
    ...
    activation="relu",input_shape=(WINDOW_IN,NODES, NODES,1)))
model.add(ConvLSTM2D(filters=16,
    kernel_size=(3, 3),
    padding="same",
    return_sequences=True,
...
model.add(ConvLSTM2D(filters=16,
    ...
    return_sequences=True,
    activation="relu"))
model.add(Conv3D(filters=1, kernel_size=(10,1,1), activation="relu"))
model.add(Reshape((NODES*NODES,1)))

model.compile(loss='mae', optimizer='adam', metrics=[RMSE(),"accuracy"])  
model.summary()
...
series_test_y=series_test_y.reshape(series_test_y.shape[0],series_test_y.shape[1])
val_forecast=val_forecast.reshape(val_forecast.shape[0],val_forecast.shape[1])
testScore1=list()
testScore2=list()
testScore3=list()
testScore4=list()
