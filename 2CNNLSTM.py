...
from keras.layers import Dense, Bidirectional,ConvLSTM2D,Conv1D, MultiHeadAttention,AdditiveAttention,Attention,TimeDistributed, Dropout, Reshape
...
data = data.reshape(data.shape[0],NODES,NODES,1)
...
# Define model
model = Sequential()
model.add(Conv2D(filters=64, kernel_size=5, activation="relu",padding="same", input_shape=(NODES,NODES,WINDOW_IN)))
